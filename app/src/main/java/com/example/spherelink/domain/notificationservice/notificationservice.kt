...
class NotificationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        job = coroutineScope.launch {
            while (isActive) {
                val deviceList: List<DeviceEntity> = repository.getDeviceEntitiesAsList()
                ...
                for (device in deviceList) {
                    ...
                    if (distance > maxDistance) {
                        Log.d(TAG, "Device out of range: ${device.name} (${device.address})")
    ...
    private fun createDistanceNotification(device: DeviceEntity) {
        Log.d(TAG, "Creating distance notification for device: ${device.name} (${device.address})")
        ...
        val notification = NotificationCompat.Builder(this, channelId)
            ...
            .setContentText("${device.name} is more than $maxDistance meters away.")
            .setSmallIcon(R.drawable.ic_notification)
